...
function ServiceDetail({ catDetail, webResDetail, webApplication, ecomDetail, hostingDetail, whiteLabelDetail, form, propertyManagementDetail, socialMediaDetail, logoDesignandBrandingDetail, printedMarketingDetail }) {
  ...
  return (
    <div>
      ...
      {catDetail[0].node.id == 'dGVybTo1' &&
        <WebApp data={webApplication} />
      }
      {catDetail[0].node.id == 'dGVybTo2NQ==' &&
        <Hosting data={hostingDetail} />
      }
      {catDetail[0].node.id == 'dGVybTo2' &&
        <Ecommerce data={ecomDetail} />
      }
      {catDetail[0].node.id == 'dGVybTo0' &&
        <SocialMediaManagement data={socialMediaDetail} />
      }
      {catDetail[0].node.id == 'dGVybTo2Ng==' &&
        <WhiteLabel data={whiteLabelDetail.pageBy.whitelabelSettings} themeOptions={whiteLabelDetail.acfOptionsThemeOptions.themeSettings} form={form} />
      }
...
export async function getStaticPaths() {
  ...
  const paths = portfolioCat.data.data.portfolioCategories.edges.map((item) => ({ params: { slug: item.node.slug } }));
...
export async function getStaticProps({ params }) {
  ...
  return {
    props: {
      ...
      webApplication: webApps.data.data.pageBy.customSystemsWebApplicationDevelopment,
      ...
      propertyManagementDetail: propertyManagement.data.data.pageBy.propertyManagement,
      logoDesignandBrandingDetail: logoDesignandBranding.data.data.pageBy.logoDesignAndBrandingSettings,
      printedMarketingDetail: printedMarketing.data.data.pageBy.printedMarketingSettings,
      form: form
    }
  }
}
